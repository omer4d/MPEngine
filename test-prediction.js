...
setInterval(function() {
	/*
	for(var i = 0; i < serverCmdBuff.length; ++i) {
		serverPlayer.moveVec.x = serverCmdBuff[i].moveX * nrandf(1, 0.03);
		serverPlayer.moveVec.y = serverCmdBuff[i].moveY * nrandf(1, 0.03);
	...
	(function(msg) {
		...
		lastCmdAck: serverCmdBuff.length > 0 ? serverCmdBuff[serverCmdBuff.length - 1].cmdNum : -1,
	...
	for(var i = 0; i < n; ++i) {
		...
		if(firstCmdNum >= 0) {
			var start, end;
			  ...
			  if(end > start) {
				console.log(serverCmdBuff[start].cmdNum, serverCmdBuff[end - 1].cmdNum);
			  serverLastAckCmd = serverCmdBuff[end - 1].cmdNum;
